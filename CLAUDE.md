...
Product images are URLs only. `Product.imageUrl` is a bare string validated as a URL (`z.string().url()`); there is no upload, no storage, no dimension/content check. It's set by `backend/prisma/seed.ts` (placeholder Lorem Picsum URLs, meant to be swapped for real ones) or an admin PATCH. The frontend `<ProductImage>` falls back to a neutral block when it's null or the URL fails to load, so a rotted link degrades quietly.
