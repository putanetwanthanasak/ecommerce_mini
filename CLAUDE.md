...
Product images are URLs only. `Product.imageUrl` is a bare string validated as a URL (`z.string().url()`); there is no upload flow and no dimension/content check. The four demo products point at files in a public Supabase Storage bucket (`product-images`), set by `backend/prisma/seed.ts`; any other product gets its `imageUrl` from an admin PATCH. The frontend `<ProductImage>` falls back to a neutral block when it's null or the URL fails to load, so a rotted link degrades quietly.

Order line items carry no image. The `GET /api/orders` and `GET /api/orders/:id` responses select `product: { id, name, price }` (see `backend/src/routes/orders.ts`), so the order-detail line thumbnails and the order-history thumbnail stack render the neutral `<ProductImage>` placeholder. Adding `imageUrl` to that `select` is a one-line change and would light both up — left as a follow-up, not done, because it was out of scope for the reference-parity pass.
