...
Status: Auth, Category, Product, and Order routes are complete and manually verified. Automated tests and CI are done — 31 Vitest/Supertest tests in backend/src/__tests__/, including the concurrent-order race, the concurrent Idempotency-Key dedupe and the login rate-limit cutoff, with GitHub Actions running typecheck plus the suite against an ephemeral Postgres on every push and PR, alongside a parallel job that lints and builds the frontend. The frontend covers auth (login, register, protected routing), the customer-facing catalog (a paginated product grid at /products with debounced search and category filtering, all reflected in the URL, plus a product detail page; `Product` carries an optional `imageUrl` rendered as a thumbnail in the list and a banner on the detail page, with a neutral fallback when null), the buying path (a client-side cart at /cart, checkout at /checkout), and the customer's order records: a paginated history at /orders and a single order at /orders/:id, which doubles as the post-checkout confirmation. No admin screens yet, and no frontend tests. Deployment config is committed — `render.yaml` (backend) and `frontend/vercel.json` — see the Deployment section below and README "Deployment".
...
9. POST /api/orders is idempotent on the `Idempotency-Key` header

The header is a UUID, required (missing or non-UUID → 400), that the client generates once per checkout attempt and reuses across every retry of it. Inside the order transaction the first write is `tx.idempotencyKey.create({ key, userId })`; the `@@unique([userId, key])` index is the guarantee — two racing retries contend on it, exactly one INSERT wins and places the order, and the loser gets `P2002` once the winner commits and replays the winner's stored response (`responseStatus` + `responseBody`, written into the same row later in the same transaction, so a committed key always carries a committed order's response). A pre-flight `SELECT` for the key would be the same stale-read bug as invariant 1 — both requests would see "no such key" and both order; the claim has to be the guarded write. A failed attempt rolls the claim back with everything else, so the key is free for a real retry. A reused key with a *different* valid body replays the original response and ignores the new body — the key identifies the operation, and the point is that no retry can create a second order. `idempotency_keys` rows are prunable after a few hours (no job yet). The client double-submit guard in CheckoutPage stays, but it is now only a UX nicety — this is the correctness mechanism, and it also covers network-level retries the button never sees.

10. POST /api/auth/login is rate limited
...
The correctness guarantee against a duplicate order is server-side now — POST /api/orders dedupes on the `Idempotency-Key` header (backend invariant 9). CheckoutPage generates one key per visit (`useState(() => crypto.randomUUID())`) and reuses it for every retry from that mount. The disabled button plus the `inFlight` ref stays, but only to spare the user a wasted round trip on a double-click; it is not what keeps stock from being decremented twice, and it never saw network-level retries anyway.
...
onRetry is deliberately omitted in two places. Login and register are retried by fixing the form and submitting it. Checkout omits it too: re-submitting is safe now (the Place order button reuses one idempotency key per visit, so a retry that already succeeded just replays the first order — invariant 11), but that button already is the retry, and a second "try again" on the banner would only compete with it.
...
Rate limiting covers only POST /api/auth/login (invariant 10). /register is unthrottled — a lower-value vector (no secret to guess), left as a follow-up. The limiter's store is per-process in memory, so a multi-instance deploy would need a shared one (Redis).
...
No payment integration. `/checkout` shows a cosmetic shipping-address + card form (matching the reference mockup), but it is labelled as taking no payment and storing no address: the fields are never read or sent, `POST /api/orders` still receives `{ productId, quantity }` per line and nothing else, and a card number ending in `0000` previews a declined state entirely client-side. There is no shipping-address column and no payment processor.
Product images are URLs only. `Product.imageUrl` is a bare string validated as a URL (`z.string().url()`); there is no upload flow and no dimension/content check. The four demo products point at files in a public Supabase Storage bucket (`product-images`), set by `backend/prisma/seed.ts`; any other product gets its `imageUrl` from an admin PATCH. The frontend `<ProductImage>` falls back to a neutral block when it's null or the URL fails to load, so a rotted link degrades quietly. Order responses carry it too: the shared `orderInclude` in `backend/src/routes/orders.ts` selects `product: { id, name, price, imageUrl }`, so order-detail lines and the order-history thumbnail stack render the real image.
