...
How this project is deployed: the API on **Render**, the SPA on **Vercel**, the database on the
existing **Supabase** instance. Config lives in two committed files — `render.yaml` at the repo root
and `frontend/vercel.json` — so the setup is reproducible rather than living only as dashboard
fields. No secrets are in either: everything sensitive is marked `sync: false` and set in the
dashboard.

Split out of the README so that stays a 30-second read. The content is unchanged from the version
that lived there; it just has its own address now.
